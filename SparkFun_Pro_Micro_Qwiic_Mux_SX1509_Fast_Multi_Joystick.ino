...
//
// 8-05-20 by Matt Geders
// Joystick Library by Matthew Heironimus
// SparkFunSX1509_Fast Library edited by Brian Charles
//--------------------------------------------------------------------
...
// Usage Example (Declaration. Just like yours but with the addition of passing in the Joystick instance):
//     Encoder myEnc(io_SX1509_3, 0, 1, Joystick1, 32, 33);
// Usage Example (in loop, naturally make sure the mux is set correctly, so just put this next to the button code for the same 1509):
//     myEnc.update();

  class Encoder {
  private:
  SX1509_Fast &an_SX1509;
  uint8_t Pin_A;
  uint8_t Pin_B;
  Joystick_ &a_joystick;
  uint8_t button_CW;
  uint8_t button_CCW;

  bool statesInitialized; // whether or not the state variables below have been set up

  uint8_t PrevPrevPrevEncoderState_A; // these are used to store the state history of the previous 3 scans on pin A
  uint8_t PrevPrevEncoderState_A;
  uint8_t PrevEncoderState_A;

  uint8_t PrevPrevPrevEncoderState_B; // these are used to store the state history of the previous 3 scans on pin B
  uint8_t PrevPrevEncoderState_B;
  uint8_t PrevEncoderState_B;

  public:
  Encoder (SX1509_Fast &an_SX1509, uint8_t Pin_A, uint8_t Pin_B, Joystick_ &a_joystick, uint8_t button_CW, uint8_t button_CCW)
    : an_SX1509(an_SX1509), a_joystick(a_joystick)
  {
    // (all of these could go in the initializer list too but only the references actually need to be there and this format might be easier to understand so I'll leave them)
    this->Pin_A = Pin_A;        // The pin on said io object you want for Pin A
    this->Pin_B = Pin_B;        // The pin on said io object you want for Pin B
    this->button_CW = button_CW;    // The JoyButton you want to associate CW with
    this->button_CCW = button_CCW;  // The JoyButton you want to associate CCW with
  ...
  void update()
  {
    uint8_t currentEncoderState_A = an_SX1509.digitalRead(Pin_A);  //Reads Encoder Pin A
    uint8_t currentEncoderState_B = an_SX1509.digitalRead(Pin_B);   //Reads Encoder Pin B

    // if this is the first update(), propagate the current state into the history
    if (statesInitialized == false)
    {
      statesInitialized = true;

      PrevPrevPrevEncoderState_A = PrevPrevEncoderState_A = PrevEncoderState_A = currentEncoderState_A;
      PrevPrevPrevEncoderState_B = PrevPrevEncoderState_B = PrevEncoderState_B = currentEncoderState_B;
    }
    else
    {    
      uint8_t Encoder_A = ((PrevPrevPrevEncoderState_A << 7) + (PrevPrevEncoderState_A << 6) + (PrevEncoderState_A << 5) + (currentEncoderState_A << 4));  //This stores the previous 3 states plus the current one in a handy way for the most accurate comparative use.
      uint8_t Encoder_B = ((PrevPrevPrevEncoderState_B << 3) + (PrevPrevEncoderState_B << 2) + (PrevEncoderState_B << 1) + (currentEncoderState_B));

      uint8_t Encoder_A1 = ((PrevPrevEncoderState_A << 5) + (PrevEncoderState_A << 4) + (currentEncoderState_A << 3));  //This stores the previous 2 states plus the current one in a handy way for a slightly more error tolerant comparative use.
      uint8_t Encoder_B1 = ((PrevPrevEncoderState_B << 2) + (PrevEncoderState_B << 1) + (currentEncoderState_B));

      uint8_t Encoder_A2 = ((PrevEncoderState_A << 3) + (currentEncoderState_A << 2));  //This stores the previous states plus the current one in a handy way for a slightly more the most error tolerant comparative use.
      uint8_t Encoder_B2 = ((PrevEncoderState_B << 1) + (currentEncoderState_B));
      
      uint8_t EncoderSum = Encoder_A + Encoder_B;  // When we compare the last 3 states of each encoder, we can determine direction of spin with a good deal of accuracy
      uint8_t EncoderSum1 = Encoder_A1 + Encoder_B1;  // A little less accurate, but it can still detect direction of spin.
      uint8_t EncoderSum2 = Encoder_A2 + Encoder_B2;  // Since this one only uses the previous state to determin direction it's the least confident way.

      // Encoder Logic Error Handling Matrix condition check and resultant action goes here.  This is for one direction.
      if ((EncoderSum) == 57 ||   // All these EncoderSum checks compare the previous 3 states to see if they match the ideal series of what should happen going from any of the 4 possible quadrature states.
          ...
          (EncoderSum2) == 11)
      {
        a_joystick.setButton(button_CW, 1);
      }
      // Encoder Logic Error Handling Matrix condition check and resultant action goes here.  This is for the other direction.
              ...
              (EncoderSum2) == 14)
      {
        a_joystick.setButton(button_CCW, 1);
      }
      ...
      if( (EncoderSum) == 255 ||  //if the encoder has been seen as stationary for the past 3 cycles including the current...
          ...
          (EncoderSum) == 240 )
      {
        a_joystick.setButton(button_CW, 0);  // ...then I guess we should let go of the joystick buttons
        a_joystick.setButton(button_CCW, 0);
      }

      PrevPrevPrevEncoderState_A = PrevPrevEncoderState_A;
...
}; // class declarations do need a semicolon even after the closing brace
...
Encoder Enc_UFC_BRT(io_SX1509_3, 0, 1, Joystick1, 32, 33);
